...
const logAnalyticsEventLocal = async (eventName, eventParams = {}) => {
  ...
  try {
    await logAnalyticsEvent(eventName, eventParams);
  } catch (firebaseError) {
    ...
    return (
      <TouchableOpacity 
        ...
        onPress={() => {
          ...
          logAnalyticsEventLocal('analytics_box_opened', {
      ...
      >
        <View style={analyticsStyles.header}>
          <View style={analyticsStyles.headerLeft}>
            ...
            <Text style={analyticsStyles.title}>Analytics Screen Analytics</Text>
          </View>
          <View style={analyticsStyles.headerRight}>
            <TouchableOpacity 
              ...
              onPress={() => {
                ...
                logAnalyticsEventLocal('analytics_cleared', { screen: 'analytics_screen' });
              }}
            >
        ...
        <View style={analyticsStyles.statsContainer}>
          ...
          <View style={analyticsStyles.statItem}>
            <Text style={analyticsStyles.statValue}>
              {analyticsEvents.filter(e => e.event.includes('sport')).length}
            </Text>
            <Text style={analyticsStyles.statLabel}>Sport Changes</Text>
          </View>
        </View>
...
const GeneralAnalyticsTab = () => {
  ...
  const USEFUL_PROMPTS = [
    ...
    },
    {
      ...
      prompts: [
        "FanDuel Snake Draft optimum picks",
        "Best value picks for NBA fantasy",
        "Top sleepers for fantasy basketball",
        "Players with highest fantasy points per game",
        "Injury replacements for fantasy lineups",
        "NBA DFS optimal lineups",
        "Underrated fantasy players 2024",
        "Fantasy basketball draft strategy",
        "FanDuel vs DraftKings scoring differences",
        "Daily fantasy sports optimal picks"
      ]
    }
  ];
  ...
  const handlePromptSelect = (prompt) => {
    ...
    logAnalyticsEventLocal('analytics_prompt_select', {
      prompt: prompt,
  ...
  const handleAnalyticsSearch = (query) => {
    ...
    if (isFantasySearch) {
      logAnalyticsEventLocal('fantasy_draft_search', {
        query: query,
        sport: selectedSport,
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        <View style={styles.searchSection}>
          <SearchBar
            placeholder={`Search ${selectedSport} games, teams, players, or fantasy drafts...`}
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.footerText}>
            Tip: Use AI Assistant for personalized insights • Change sport for different analytics
          </Text>
        </View>
      </ScrollView>
...
export default function AnalyticsScreenEnhanced({ navigation, route }) {
...
const styles = StyleSheet.create({
  ...
  refreshIndicator: {
    ...
    justifyContent: 'center',
    paddingVertical: 12,
    backgroundColor: '#ffffff', // FIXED: Explicit solid color added
    borderBottomWidth: 1,
    borderBottomColor: '#e5e7eb',
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
  ...
  metricTab: {
    ...
    borderRadius: 12,
    minWidth: 70,
    backgroundColor: '#f8fafc', // FIXED: Added solid background color
  },
  metricTabContent: {
    flexDirection: 'row',
    alignItems: 'center',
