  ...
  const loadAnalyticsEvents = async () => {
    try {
      ...
      if (eventsString) {
        ...
        // Get only the 10 most recent predictions-related events
        const predictionsEvents = events.filter(event => 
          event.event.includes('predictions') || 
          event.event.includes('generation') ||
          event.event.includes('prompt') ||
          event.event.includes('pick')
        ).slice(-10).reverse();
        setAnalyticsEvents(predictionsEvents);
      }
    } catch (error) {
      ...
      >
        <View style={analyticsStyles.header}>
          <View style={analyticsStyles.headerLeft}>
            ...
            <Text style={analyticsStyles.title}>Predictions Analytics</Text>
          </View>
          <View style={analyticsStyles.headerRight}>
            <TouchableOpacity 
              ...
              onPress={() => {
                ...
                logAnalyticsEvent('analytics_cleared', { screen: 'predictions' });
              }}
            >
        ...
        <View style={analyticsStyles.statsContainer}>
          ...
          <View style={analyticsStyles.statItem}>
            <Text style={analyticsStyles.statValue}>
              {analyticsEvents.filter(e => e.event.includes('generation')).length}
            </Text>
            <Text style={analyticsStyles.statLabel}>Generations</Text>
          </View>
          <View style={analyticsStyles.statItem}>
            <Text style={analyticsStyles.statValue}>
              {analyticsEvents.filter(e => e.event.includes('prompt')).length}
            </Text>
            <Text style={analyticsStyles.statLabel}>Prompts</Text>
          </View>
        </View>

        <View style={analyticsStyles.eventsContainer}>
          <Text style={analyticsStyles.eventsTitle}>Recent Predictions Events</Text>
          <ScrollView style={analyticsStyles.eventsList}>
            {analyticsEvents.length === 0 ? (
              <View style={analyticsStyles.emptyEvents}>
                <Ionicons name="trending-up-outline" size={40} color="#475569" />
                <Text style={analyticsStyles.emptyText}>No predictions analytics recorded</Text>
                <Text style={analyticsStyles.emptySubtext}>Generate predictions to see events</Text>
              </View>
            ) : (
        ...
        <View style={analyticsStyles.buttonContainer}>
          <TouchableOpacity 
            style={analyticsStyles.refreshButton}
            onPress={loadAnalyticsEvents}
          >
            <Ionicons name="refresh" size={16} color="white" />
            <Text style={analyticsStyles.refreshButtonText}>Refresh</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            style={analyticsStyles.statsButton}
            onPress={() => {
              const generationEvents = analyticsEvents.filter(e => e.event.includes('generation'));
              const avgPromptLength = generationEvents.length > 0 
                ? generationEvents.reduce((sum, e) => sum + (e.params.prompt?.length || 0), 0) / generationEvents.length
                : 0;
              
              Alert.alert(
                'Prediction Stats',
                `Total Generations: ${generationEvents.length}\nAvg Prompt Length: ${avgPromptLength.toFixed(0)} chars\nSuccess Rate: ${(generationEvents.filter(e => e.event.includes('success')).length / generationEvents.length * 100).toFixed(0)}%`,
                [{ text: 'OK' }]
              );
            }}
          >
            <Ionicons name="stats-chart" size={16} color="white" />
            <Text style={analyticsStyles.statsButtonText}>Stats</Text>
          </TouchableOpacity>
        </View>
      </LinearGradient>
    </View>
...
export default function PredictionsScreen() {
  ...
  const [advancedAnalytics, setAdvancedAnalytics] = useState([
    "📊 Use 'trending' in prompts for picks with upward momentum",
    "💰 Combine multiple sports for diversified parlays",
    "🎯 Target player props with >85% historical hit rate",
    "📈 Look for line movements of 2+ points for value",
    "🔍 Search specific players facing weak defenses",
    "⚡ Use 'hot streak' in prompts for in-form players",
    "🎲 Include 'value bet' for underrated opportunities",
    "🏆 Focus on primetime games for higher volume data"
  ]);
  ...
  const handleNavigateToExpertSelections = () => {
    ...
    logAnalyticsEvent('predictions_navigate_expert_selections', {
  ...
  const handleNavigateToPlayerMetrics = () => {
    navigation.goToSuccessMetrics();
    logAnalyticsEvent('predictions_navigate_player_metrics', {
      screen_name: 'Predictions Screen'
    });
  };
  ...
  const handleNavigateToParlayBuilder = () => {
    ...
    logAnalyticsEvent('predictions_navigate_parlay', {
      screen_name: 'Predictions Screen'
    });
  };
  ...
  const handleNavigateToPremium = () => {
    navigation.goToSuccessMetrics();
    logAnalyticsEvent('predictions_navigate_premium', {
      screen_name: 'Predictions Screen'
    });
  };

  // Enhanced Random Prompts System - INCLUDING FANDUEL SNAKE DRAFT
  const getRandomPrompts = () => {
    const allPrompts = [
      ...
      // FANDUEL SNAKE DRAFT SPECIFIC
      "Generate FanDuel Snake Draft optimum picks for NBA tonight",
      "FanDuel Snake Draft optimal lineup suggestions",
      "Best Snake Draft strategy picks for FanDuel contests",
      "FanDuel Snake Draft player rankings and optimal picks",
      "Generate winning FanDuel Snake Draft lineup",
      "Snake Draft player value picks for FanDuel",
      "Optimal Snake Draft selections for FanDuel NBA",
      "FanDuel Snake Draft sleepers and value picks",
  ...
  // Enhanced mock picks data with dual analysis - INCLUDING FANDUEL SNAKE DRAFT PICKS
  const mockPicks = [
    {
      id: '1',
    ...
    },
    {
      ...
      team: 'DEN',
      sport: 'NBA',
      pick: 'FanDuel Snake Draft Top Pick',
      confidence: 95,
      odds: '-150',
      edge: '+7.1%',
      analysis: 'Advanced: FanDuel Snake Draft optimal selection. Jokic leads all players in fantasy points per game (62.3). Triple-double threat every night. Facing team allowing 115.2 PPG to centers. Usage rate 31.4% with elite efficiency.',
      simpleAnalysis: 'Easy Explanation: Jokic is the best fantasy basketball player right now. In FanDuel Snake Drafts, he should be your #1 pick every time.',
    ...
    },
    {
      ...
      team: 'MIA',
      sport: 'NFL',
      pick: 'FanDuel Snake Draft WR1',
      confidence: 91,
      odds: '-125',
      edge: '+6.3%',
      analysis: 'Advanced: FanDuel Snake Draft optimal WR selection. Hill averaging 28.4 fantasy points per game (leads all WRs). Targets trending upward (12.2 per game). Facing 32nd ranked pass defense. Breakaway speed creates ceiling games.',
      simpleAnalysis: 'Easy Explanation: Hill is the top wide receiver in fantasy football. In FanDuel Snake Drafts, he\'s worth drafting early as your WR1.',
  ...
  // Enhanced pick generation with dual analysis - INCLUDES FANDUEL SNAKE DRAFT
  const generatePicksBasedOnPrompt = (prompt) => {
    const timestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    const now = Date.now();
    
    // Generate advanced and simple analysis
    const generateDualAnalysis = (sport, player, prop, isSnakeDraft = false) => {
      if (isSnakeDraft) {
        const snakeDraftAnalyses = [
          `Advanced: FanDuel Snake Draft optimal selection. ${player} leads in fantasy points per game. Usage rate ${Math.floor(25 + Math.random() * 15)}% with elite efficiency. Top-tier value in Snake Draft format.`,
          `Advanced: Ideal FanDuel Snake Draft pick. Projected for ${Math.floor(40 + Math.random() * 25)} fantasy points. Position scarcity makes this a high-value selection in Snake Draft contests.`,
          `Advanced: Snake Draft strategy pick for FanDuel. ${player} provides consistent floor with high ceiling. Optimal selection based on draft position and opponent matchup.`
        ];
        
        const simpleSnakeAnalyses = [
          `Easy Explanation: Great FanDuel Snake Draft pick. ${player} is consistently one of the top fantasy performers and should be drafted early.`,
          `Easy Explanation: In FanDuel Snake Drafts, ${player} offers excellent value at this draft position.`,
          `Easy Explanation: Strong Snake Draft selection for FanDuel. ${player} has a favorable matchup and high fantasy potential.`
        ];
        
        return {
          advanced: snakeDraftAnalyses[Math.floor(Math.random() * snakeDraftAnalyses.length)],
          simple: simpleSnakeAnalyses[Math.floor(Math.random() * simpleSnakeAnalyses.length)]
        };
      }
    ...
    // Check if prompt includes FanDuel Snake Draft
    const isSnakeDraftPrompt = prompt.toLowerCase().includes('snake') || prompt.toLowerCase().includes('fanduel');
    ...
    for (let i = 0; i < pickCount; i++) {
      ...
      const players = {
        NBA: ['LeBron James', 'Stephen Curry', 'Kevin Durant', 'Giannis Antetokounmpo', 'Jayson Tatum', 'Nikola Jokic', 'Luka Doncic'],
        NFL: ['Patrick Mahomes', 'Josh Allen', 'Justin Jefferson', 'Travis Kelce', 'Christian McCaffrey', 'Tyreek Hill', 'Ja\'Marr Chase'],
        MLB: ['Shohei Ohtani', 'Aaron Judge', 'Ronald Acuña Jr.', 'Mookie Betts', 'Bryce Harper'],
        NHL: ['Connor McDavid', 'Nathan MacKinnon', 'Auston Matthews', 'Leon Draisaitl', 'David Pastrňák']
      };
      ...
      const isSnakeDraft = isSnakeDraftPrompt && Math.random() > 0.5;
      const pickType = isSnakeDraft ? 'FanDuel Snake Draft Optimal Pick' : `Generated ${sport} Prediction`;
      
      const analysis = generateDualAnalysis(sport, player, 'prop', isSnakeDraft);
  ...
  useEffect(() => {
    loadPicks();
    logAnalyticsEvent('predictions_screen_view', { sport: selectedSport });
  }, [selectedSport]);
  ...
  const onRefresh = async () => {
    ...
    setUsefulPrompts(getRandomPrompts());
    logAnalyticsEvent('predictions_refresh', { sport: selectedSport });
  };
  ...
  const renderPromptsSection = () => (
    <View style={styles.promptsSection}>
      <View style={styles.promptsHeader}>
        <View>
          <Text style={styles.promptsTitle}>🚀 AI Prediction Generator</Text>
      ...
      <View style={styles.customPromptContainer}>
        <View style={styles.promptInputContainer}>
          ...
          <TextInput
            ...
            placeholder="Or type your own prompt (e.g., 'FanDuel Snake Draft optimum picks for NBA')"
      ...
      <View style={styles.promptsFooter}>
        ...
        <Text style={styles.promptsFooterText}>
          Uses advanced AI to analyze trends and generate high probability predictions including FanDuel Snake Draft optimum picks
        </Text>
      </View>
    </View>
  );
        ...
        <View style={styles.pickCardContent}>
            ...
            <View style={styles.playerInfo}>
              <View>
                ...
                <View style={styles.pickSubheader}>
                  ...
                  {item.isSnakeDraft && (
                    <View style={styles.snakeDraftBadge}>
                      <Ionicons name="trophy" size={12} color="#f59e0b" />
                      <Text style={styles.snakeDraftText}>Snake Draft</Text>
                    </View>
                  )}
                </View>
              </View>
          ...
          <View style={styles.analysisContainer}>
            ...
            <Text style={[styles.analysisText, isPremiumLocked && styles.premiumLockedText]}>
              {isPremiumLocked ? '🔒 Premium prediction analysis available with upgrade' : currentAnalysis}
            </Text>
          </View>
          ...
          <View style={styles.footer}>
            ...
            <TouchableOpacity 
              ...
              onPress={() => {
                ...
                logAnalyticsEvent('predictions_track_pick', {
                  player: item.player,
                  pick: item.pick,
                  confidence: item.confidence,
                  isSnakeDraft: item.isSnakeDraft || false
                });
    ...
    <View style={styles.analyticsTipsSection}>
      ...
      <View style={styles.parlayTip}>
        ...
        <Text style={styles.parlayTipText}>
          💡 Pro Tip: Try "FanDuel Snake Draft optimum picks" for optimal fantasy draft selections
        </Text>
      </View>
    </View>
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.statsSection}>
          <Text style={styles.sectionTitle}>📈 Prediction Performance</Text>
          ...
          <View style={styles.sectionHeader}>
            <View>
              ...
              <Text style={styles.pickCount}>
                {filteredPicks.length} predictions • {predictionCounter.remainingPredictions} free left today
              </Text>
            </View>
          ...
          {filteredPicks.length > 0 ? (
            ...
            <View style={styles.emptyContainer}>
              ...
              <TouchableOpacity 
                ...
                onPress={() => generateHighProbabilityPicks('Generate FanDuel Snake Draft optimum picks for me')}
                disabled={generating}
              >
...
const styles = StyleSheet.create({
  ...
  navButtonContent: {
    ...
    alignItems: 'center',
    width: '100%',
    backgroundColor: 'rgba(139, 92, 246, 0.2)', // FIXED: Added background color
  },
  ...
  sportButtonActive: {
    backgroundColor: 'rgba(139, 92, 246, 0.3)', // FIXED: Changed from transparent to solid color
  },
  ...
  sportButtonContent: {
    ...
    width: '100%',
    justifyContent: 'center',
    backgroundColor: 'rgba(0,0,0,0.3)', // FIXED: Added background color
  },
  ...
  promptChip: {
    ...
    borderRadius: 20,
    elevation: 3,
    backgroundColor: '#8b5cf6', // FIXED: Added background color
  },
  ...
  statBox: {
    ...
    marginBottom: 15,
    elevation: 3,
    backgroundColor: '#8b5cf6', // FIXED: Added default background color
  },
  ...
  analyticsTipsSection: {
    ...
    backgroundColor: '#1e293b',
    elevation: 5,
    shadowColor: '#000', // FIXED: Added shadow properties
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.2,
    shadowRadius: 8,
  },
  ...
  sportText: {
    fontSize: 11,
    fontWeight: '600',
    color: 'white', // FIXED: Added text color
  },
  ...
  confidenceBadge: {
    ...
    paddingVertical: 8,
    borderRadius: 20,
    backgroundColor: '#10b981', // FIXED: Added background color
  },
  ...
  trackButton: {
    ...
    paddingVertical: 8,
    borderRadius: 12,
    backgroundColor: 'rgba(139, 92, 246, 0.1)', // FIXED: Added background color
  },
